...
You may change among these optimizers using the 'optimizer' parameter, where:
- SGD - optimizer = 1
- ADAM - optimizer = 2
- AMSGrad - optimizer = 3
...
Program is compiled using the following command:
  ...
  nvcc SGD_CUDA_eng.c functions.c -o program.out -lcublas
...
double sgd(int M, int N, int M_v, int batch_size, int iter, double lr, double gradient_tolerance,FILE *f, int optimizer, double beta_adam_1, double beta_adam_2, double epsilon) {
  ...
  array_1d_T y, y_v, b, g, y_b, mt, vt, tmp1, tmp2, y_static, y_v_static, rmse, rmse_v, vt_hat, time_vec;
...
// Reserve required space
  ...
  time_vec=malloc(sizeof(*time_vec));
...
// Reserve memory in host
  ...
  values_vector(time_vec)=malloc(iter*sizeof(double));
...
while(it <= iter && grad_norm > gradient_tolerance && acum/M <10000){
...
// Calculate gradient : g = -X^t %*% e
  ...
  if(optimizer == 1){
    ...
    // Calculate mt = B1*mt-1 + (1-B1)*gt
        ...
        for(int i =0; i<N; i++){
            value_vector(tmp1,i) = value_vector(g,i)*value_vector(g,i);
        }
        stat = cublasSetVector (N , sizeof (double ) ,tmp1->arr ,1 , d_tmp1 ,1);
        ...
        stat = cublasGetVector (N , sizeof (double ) , d_vt ,1 ,vt->arr ,1);
         ...
         if(optimizer ==2){
          for(int i =0; i< N; i++){
            double mt_hat = value_vector(mt,i)/(1.0-pow(beta_adam_1,it));
            double vt_hat = value_vector(vt,i)/(1.0-pow(beta_adam_2,it));
            value_vector(b,i) = value_vector(b,i) - lr*mt_hat/(sqrt(vt_hat)+epsilon);
           }
        }
        else{
          for(int i=0; i<N; i++){
            value_vector(vt_hat,i) = MAX(value_vector(vt,i), value_vector(vt_hat,i));
          }
          for(int i =0; i< N; i++){
            value_vector(b,i) = value_vector(b,i) - lr*value_vector(mt,i)/(sqrt(value_vector(vt_hat,i))+epsilon);
          }
        }
        stat = cublasSetVector (N , sizeof (double ) ,b->arr ,1 , d_b ,1);
      }
  ...
  value_vector(time_vec, it) = time_spent;
  printf("Iteration execution time: %f\n", time_spent);
  printf("------------\n");
  fprintf(f, "%d,%d, %f, %f, %f ,%f, %f\n", it, iter, acum/M, acum_v/M_v,grad_norm, lr, time_spent);
  it++;
  // Add decay to iterations
  // 수정 3) learning rate decay 제거
  // lr = lr * 0.999;

}
printf("----- End with learning rate of %lf -----\n", lr);
...
int main(int argc, char const *argv[]) {
  ...
  // 수정 1) gradient_tolerance 값 변경 (1e-12 -> 1e-6)
  double gradient_tolerance = 1e-6;
  int optimizer=atoi(argv[6]);
  double beta_adam_1=atof(argv[7]);
  double beta_adam_2=atof(argv[8]);
  double epsilon=atof(argv[9]);
  // Save csv
  FILE *f = fopen("error_sgd_cuda.csv", "w");
  FILE *f2 = fopen("error_sgd_cuda_summary.csv", "w");

  fprintf(f2, "learning_rate, loss\n");
  fprintf(f, "iteration, tot_iter, rss_train, rss_val, grad_norm, lr, time\n");
  for(int i = 0; i<sizeof(lrs)/sizeof(lrs[0]); i++){
    loss[i] = sgd(M, N, M_v, batch_size, iter, lrs[i],gradient_tolerance,f, optimizer, beta_adam_1, beta_adam_2, epsilon);
  }
