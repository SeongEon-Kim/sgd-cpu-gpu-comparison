/*
Sequential Stochastic Gradient Descent with Time Measurement
*/
...
// Calculation of the training and validation errors
  ...
  // 반복 종료 시간 측정
  end_time = clock();
  iteration_time = ((double)(end_time - start_time)) / CLOCKS_PER_SEC;
  cumulative_time += iteration_time;

  printf("Iteration %d/%d RMSE train: %lf -- RMSE val: %lf \n", it+1, iter, sqrt(value_vector(rmse,it)), sqrt(value_vector(rmse_v,it)));
  printf("Iteration execution time: %lf\n", cumulative_time);
  printf("------------\n");
...
FILE *f = fopen("sgd_output_time.txt", "w");
if (f == NULL)
{
